...
namespace iLabs.ServiceBroker.iLabSB
{
    public partial class MyClient : System.Web.UI.Page
    {
        ...
        protected void HelpButton_Click(object sender, CommandEventArgs e)
        {
            ...
            ClientScript.RegisterStartupScript(this.GetType(), "DocsPopup", jScript); 
        }
        ...
        private void launchLabClient(int c_id)
        {
            ...
            if (clients.Length > 0)
            {
                ...
                else if (clients[0].clientType == LabClient.INTERACTIVE_APPLET)
                {
                    ...
                    ClientScript.RegisterStartupScript(this.GetType(), "ReloadFrame", jScript);
                }
...
#else
                        //
                        // Determine the URL to launch - the URL query string may already contain some parameters
                        //
                        string url = lc.loaderScript + ((lc.loaderScript.Contains("?")) ? "&" : "?");
                        url += "couponID=" + coupon.couponId + "&passkey=" + coupon.passkey;

                        string jScript = @"<script language='javascript'> window.open ('" + url + "')</script>";
                        //Page.RegisterStartupScript("HTML Client", jScript);
                        ClientScript.RegisterStartupScript(this.GetType(), "HTML Client", jScript);
#endif
                    }
                }
                // use the Loader script for Batch experiments
...
#else
                    //
                    // Put the loader script onto the web page
                    //
                    phBatchApplet.Controls.Add(new LiteralControl(clients[0].loaderScript));
#endif
                }
            }
            else
